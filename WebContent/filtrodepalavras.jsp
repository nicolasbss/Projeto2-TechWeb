...
<%	DAO dao = new DAO();
	String palavra_filtrada = (String) request.getAttribute("palavra_filtrada");
...
<body>	
	<ul>
		...
		<li><a href= '#'>
			<h2>Nota</h2>
			<p><%=nota.getConteudo()%><br>
			Criado em <%=dao.horarioCriacao(nota)%></p>
	...
	<form action="OrdenaNotas">
		<input type="hidden" name='pessoa_id' value=<%=id_usuario %>>
		<input type = "submit" value="Ordenar por ordem de atualização">
	</form>
	
	<form action="ListaNotas">
		<input type="hidden" name='pessoa_id' value=<%=id_usuario %>>
		<input type = "submit" value="Ordenar por criacao">
	</form>
